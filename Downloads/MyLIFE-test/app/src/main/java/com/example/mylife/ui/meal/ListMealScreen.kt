...
@Composable
fun MyLazyColumn(navigateToEachMeal: (Int) -> Unit, mealList: List<Meal>) {
    Text(text = "List Meal:",
        modifier = Modifier.padding(0.dp, 0.dp, 0.dp, 10.dp),
        fontSize = 30.sp,
        fontWeight = FontWeight.Bold)
    LazyColumn() {
        items(items = mealList, key = {it.meal_id}){
            meal -> Item(meal = meal, navigateToEachMeal)
        }
    }
}
...
//                currentScreen: AppScreen
        ...
        Column(horizontalAlignment = Alignment.End,
            ...
            Column() {
                MyLazyColumn(navigateToEachMeal, mealList)
            }
