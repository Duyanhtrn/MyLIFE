...
@Composable
    fun AppNavHost(
        ...
        viewModel: navViewModel = viewModel(factory = AppViewModelProvider.Factory)
    ) {
        NavHost(
            navController = navController,
            startDestination = viewModel.start,
            modifier = modifier
        ) {
        composable(route = WelcomeDestination.route) {
            WelcomeScreen(navigateToEntryInfor={ navController.navigate(EntryInforDestination.route) },
                )
            viewModel.start = HomeDestination.route
        }
        ...
        composable(
            route = EachMealDestination.route,
